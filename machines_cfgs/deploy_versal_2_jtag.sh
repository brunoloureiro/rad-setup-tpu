#!/usr/bin/env bash
# deploy_versal_2_jtag.sh - (Re)deploy a Versal bare-metal PDI+ELF over JTAG, targeting board 2.
#
# Same as deploy_versal_jtag.sh (see that file for the full rationale), except this wrapper pins
# BOARD=2 so debug.tcl/deploy.tcl (via versal_scripts/board_select.tcl) target the second board in
...
# 'redeploy_cmd' for one specific physical board (see versal2_rowA14_colA14_colB14_N34.yaml) so it
...
# config (see machines_cfgs/versal2_rowA14_colA14_colB14_N34.yaml and CLAUDE.md's "DUT
