...
with the ergonomics described in "CLI ergonomics" below, and the per-machine Monitor thread
(`server/monitors/` - base class, registry, one example implementation - see "Monitor thread API"
below for the now-resolved design and machine.py's `__start_monitor`/`__current_log_file`).
Covered by hermetic tests: `tests/test_machine_commands.py`, `tests/test_command_cli.py`,
`tests/test_machine_operator_commands.py`, `tests/test_monitors.py`,
`tests/test_machine_monitor.py`, `tests/test_server_ctrlc.py`.

## CLI ergonomics

- **Immediate, specific feedback on bad input.** `machine_commands.py`'s `COMMAND_PARAMS` (a
  `ParamSpec` per expected parameter: name, a `validate(raw) -> normalized_value` function, and a
  one-line `hint`) plus `validate_command_params()` are the single source of truth for each
  command's generic parameter syntax, shared by `command_cli.py` and reused by `Machine`'s own
  handlers (e.g. `__on_operator_sleep`) - so the same "missing"/"invalid"/"ignoring an
  unrecognized parameter" checks run both while the operator is still typing and, defensively,
  again inside `Machine.command(...)` (which can be reached by anything, not only the CLI).
  Semantic, DUT-specific checks (e.g. "is this codename one *this* DUT actually has") stay in
  `Machine`'s own handler, since they have no meaning outside one specific machine's state.
- **`help`/`?`** at any prompt shows context-appropriate help: the full command list
  (`describe_commands()`) at the top level or while being prompted for a command, the list of
  known DUTs (`describe_known_duts()`) while being prompted for a dut, or a parameter's one-line
  `hint` while being prompted for that parameter.
- **Partial input, filled in interactively.** A line no longer has to supply `dut`, `command`,
  and every parameter all at once - `parse_command_line()` returns whatever it found (`dut`/
  `command` come back `None` if omitted, in which case `InteractiveCommandCLI` prompts for them;
  a missing/invalid parameter is prompted for individually too, showing the reason if a value was
  given but rejected). A line is only rejected outright (`CommandLineError`) for a genuine syntax
  problem - unbalanced quoting, an explicitly-given but unrecognized command, too many positional
  arguments - never merely for something being absent.
- **Cancel words** (`q`/`quit`/`cancel`/`exit`, case-insensitive, reserved at every prompt)
  abandon whatever command entry is in progress and return to a fresh prompt, without exiting the
  CLI thread itself.
- **`InteractiveCommandCLI.stop()`** ends `run()` within one `_READ_POLL_INTERVAL_SECONDS`
  (~0.2s) even mid multi-step prompt, never affecting the server or any `Machine` - this is what
  makes the two-stage Ctrl+C behavior below responsive. It reads stdin by polling the raw fd with
  `select()` and doing its own line-buffering (`__read_line`), deliberately *not* via a plain
  `sys.stdin.readline()`: a buffered `TextIOWrapper`'s `readline()` can silently pull everything
  currently sitting in the OS pipe/tty buffer into its own internal buffer on one call (e.g.
  several lines arriving close together, as `tests/test_command_cli.py`'s piped-stdin integration
  tests do) - after which `select()` on the raw fd reports "nothing new" even though a further
  `readline()` would still happily return the next already-buffered line without blocking. Hence
  the byte-level `os.read()` + manual `\n`-splitting instead.
- **Two-stage Ctrl+C** (`server.py`): the first Ctrl+C stops the interactive CLI *only* -
  `INTERACTIVE_CLI.stop()` - never the server, since an operator's reflexive double-tap must not
  be able to end a whole radiation run. A further Ctrl+C stops the server, but only once at least
  `cli_shutdown_confirm_delay` seconds (a new optional `server_parameters.yaml` field, default
  5.0) have passed since the CLI was stopped - an earlier one is logged and ignored. `--enable_curses`
  runs have no CLI to stop, so a single Ctrl+C there still shuts down immediately, unchanged. The
  actual decision (`_decide_sigint_action`) is a small, pure, unit-tested function kept separate
  from the real signal handler specifically so it doesn't need real signals/threads/`sys.exit()`
  to test - see `tests/test_server_ctrlc.py`.
